...
impl FieldElement51 {
    ...
    fn batch_subtract_avx2<const N: usize>(a: &mut [Self; N], b: &Self) {
        ...
        let right = FieldElement2625x4::splat(&b_neg);
        while i + 3 < N {
    ...
    fn batch_add_avx2<const N: usize>(a: &mut [Self; N], b: &Self) {
        let mut i = 0;
        let right = FieldElement2625x4::splat(b);
        while i + 3 < N {
            let left = FieldElement2625x4::new(&a[i], &a[i + 1], &a[i + 2], &a[i + 3]);
    ...
    fn batch_square_avx2<const N: usize>(a: &mut [Self; N]) {
        ...
        while i + 3 < N {
            let results = FieldElement2625x4::new(&a[i], &a[i + 1], &a[i + 2], &a[i + 3])
                .square()
                .split();
    ...
    fn batch_add_avx512<const N: usize>(a: &mut [Self; N], b: &Self) {
        ...
        while i + 3 < N {
            let a_vec = F51x4Unreduced::new(&a[i], &a[i + 1], &a[i + 2], &a[i + 3]);
            let b_vec = F51x4Unreduced::new(b, b, b, b);
            let result: F51x4Reduced = a_vec.sum(&b_vec).into();
            let splits = F51x4Unreduced::from(result).split();
            a[i] = splits[0];
            a[i + 1] = splits[1];
            a[i + 2] = splits[2];
