...
impl FieldElement51 {
        ...
        while i + 3 < N {
            unsafe {
                for limb_idx in 0..5 {
                    ...
                    let p_multiple = _mm256_set1_epi64x(
                        (36028797018963952u64 - (limb_idx == 0) as u64 * 288) as i64,
                    );
    ...
    fn batch_mul_simd<const N: usize>(a: &mut [Self; N], b: &[Self; N]) {
        ...
        while i + 3 < N {
            let results = Self::batch_mul_4way_avx2(
    ...
    fn batch_mul_4way_avx2(a: &[Self; 4], b: &[Self; 4]) -> [Self; 4] {
        ...
        fn mul64_u64x4(x: u64x4, y: u64x4) -> (u64x4, u64x4) {
            ...
            (unsafe { core::mem::transmute(lo_arr) }, unsafe {
                core::mem::transmute(hi_arr)
            })
        }
