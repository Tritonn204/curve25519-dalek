...
impl<'a, 'b> Mul<&'b F51x4Reduced> for &'a F51x4Reduced {
    ...
    /// Batch subtract: for each i, a[i] -= b
    /// Uses AVX512IFMA intrinsics to process 4 field elements at once
    /// Processes multiple F51x4Reduced in parallel for maximum SIMD utilization
    #[unsafe_target_feature("avx512ifma,avx512vl")]
    pub fn batch_subtract<const N: usize>(a: &mut [Self; N], b: &Self) {
        ...
        for chunk_idx in 0..chunks {
            ...
            let a0 = F51x4Unreduced::from(a[base]);
        ...
        for i in 0..remainder {
            let diff = F51x4Unreduced::from(a[base + i]).diff(&b_unreduced);
            a[base + i] = Self::from(diff);
        }
    }

    /// Batch add: for each i, a[i] += b
    /// Uses AVX512IFMA intrinsics to process 4 field elements at once
    /// Processes multiple F51x4Reduced in parallel for maximum SIMD utilization
    #[unsafe_target_feature("avx512ifma,avx512vl")]
    pub fn batch_add<const N: usize>(a: &mut [Self; N], b: &Self) {
        ...
        let base = chunks * UNROLL;
        for i in 0..remainder {
            let sum = F51x4Unreduced::from(a[base + i]).sum(&b_unreduced);
            a[base + i] = Self::from(sum);
        }
    }

    /// Batch multiply: for each i, a[i] *= b[i]
    /// Uses AVX512IFMA intrinsics to process 4 field elements at once
    /// Processes multiple F51x4Reduced in parallel for maximum SIMD utilization
    #[unsafe_target_feature("avx512ifma,avx512vl")]
    pub fn batch_mul<const N: usize>(a: &mut [Self; N], b: &[Self; N]) {
        ...
        let base = chunks * UNROLL;
        for i in 0..remainder {
            a[base + i] = Self::from(a[base + i].mul(&b[base + i]));
        }
    }

    /// Batch square: for each i, a[i] = a[i]^2
    /// Uses AVX512IFMA intrinsics to process 4 field elements at once
    /// Processes multiple F51x4Reduced in parallel for maximum SIMD utilization
    #[unsafe_target_feature("avx512ifma,avx512vl")]
    pub fn batch_square<const N: usize>(a: &mut [Self; N]) {
        ...
        let base = chunks * UNROLL;
        for i in 0..remainder {
            a[base + i] = Self::from(a[base + i].square());
        }
    }
}
...
mod test {
    ...
    fn batch_mul_matches_serial() {
        ...
        let mut batch_a = [
            F51x4Unreduced::new(&a, &a, &a, &a).into(),
            F51x4Unreduced::new(&b, &b, &b, &b).into(),
    ...
    #[test]
    fn batch_square_matches_serial() {
        let a = FieldElement51([2438, 24, 243, 0, 0]).invert();
        let b = FieldElement51([98098, 87987897, 0, 1, 0]).invert();
        let c = FieldElement51([12345, 67890, 11111, 22222, 33333]).invert();
    ...
    #[test]
    fn batch_operations_large_array() {
